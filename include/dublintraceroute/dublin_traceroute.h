...
private:
	const uint16_t srcport_, dstport_;
	const std::string dst_;
	const probe_type type_;
	Tins::IPv4Address target_;
	const uint8_t npaths_, min_ttl_, max_ttl_;
	const uint16_t delay_;
	const bool broken_nat_, use_srcport_for_path_generation_, no_dns_;
	std::mutex mutex_tracerouting, mutex_sniffed_packets;
...
public:
	...
	DublinTraceroute(
		...
		const bool no_dns = default_no_dns) : dst_(dst),
											  type_(type),
											  srcport_(srcport),
											  dstport_(dstport),
											  npaths_(npaths),
	...
	DublinTraceroute(
		...
		const bool broken_nat = default_broken_nat,
		const bool use_srcport_for_path_generation = default_use_srcport_for_path_generation,
		const bool no_dns = default_no_dns) : dst_(std::string(dst)),
											  type_(type),
											  srcport_(srcport),
											  dstport_(dstport),
											  npaths_(npaths),
	...
	DublinTraceroute(const DublinTraceroute &source) : dst_(source.dst_),
													   type_(source.type_),
